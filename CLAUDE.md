...
- **Delete completed TODO files immediately.** When all items in a `TODO_N.md` (or `TODO_N_*.md`) file are done, delete the file in the same response. Update `ROADMAP.md` with the completion summary first. Completed TODO files are ephemeral work-tracking artifacts, NOT permanent documentation. Leaving them on disk creates confusion about what work remains.
